...
import { maskSensitiveInto } from './sensitive';

/** 工具结果增强渲染（对标 DSH output.render 轻量版）：
 *  结果若是 JSON（含 records/rows/tbody 等数组），自动表格化展示；
 *  否则转义原文。mask=true 时对凭据/密钥类敏感串做可切换显隐脱敏。
 *  全部内容已转义，无 XSS 风险。 */
export function renderToolResult(text: string, mask = false): string {
  const esc = (s: string) => (mask ? maskSensitiveInto(s) : escapeHtml(s));
  if (!text || !text.trim().startsWith('{')) return esc(text);
  let obj: unknown;
  try {
    obj = JSON.parse(text);
  } catch {
  ...
  if (typeof arr[0] === 'object' && arr[0] !== null && !Array.isArray(arr[0])) {
    ...
    html = '<table class="md-table"><thead><tr>' +
      keys.map((k) => `<th>${esc(String(k))}</th>`).join('') +
      '</tr></thead><tbody>';
    for (const r of slice) {
      html += '<tr>' + keys.map((k) =>
        `<td>${esc(String((r as Record<string, unknown>)[k] ?? ''))}</td>`).join('') + '</tr>';
    }
    html += '</tbody></table>';
    ...
    html = '<table class="md-table">' +
      (head ? '<thead><tr>' + head.map((h: unknown) =>
        `<th>${esc(String(h))}</th>`).join('') + '</tr></thead>' : '') +
      '<tbody>';
