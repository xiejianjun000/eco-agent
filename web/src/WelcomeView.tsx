      ...
      <div className="quick-tags">
        {QUICK_TAGS.map((t) => (
          <button
            key={t.label}
            className="quick-tag"
            onClick={() => window.location.assign(`/chat?q=${encodeURIComponent(t.label)}`)}
            title={t.desc}
          >
        ...
        <div className="skill-cards-scroll">
          {SKILL_CARDS.map((s) => (
            <button
              ...
              onClick={() => window.location.assign(`/chat?q=${encodeURIComponent(s.label)}`)}
            >
        ...
        <div className="welcome-input-box">
          <textarea
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter" && !e.shiftKey) {
                e.preventDefault();
                if (query.trim()) window.location.assign(`/chat?q=${encodeURIComponent(query.trim())}`);
              }
            }}
          />
          <div className="welcome-input-toolbar">
            ...
            <div className="wit-right">
              ...
              <button
                className="wit-send"
                onClick={() => query.trim() && window.location.assign(`/chat?q=${encodeURIComponent(query.trim())}`)}
              >
                ➤
              </button>
            </div>
