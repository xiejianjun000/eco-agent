...
ROLES = {
    ...
    "law": {
        ...
        "brief": (
            "你是法规核验专家。针对任务核验适用法律法规：给出真实现行有效的法规名称与具体条款号，"
    ...
    "doc": {
        ...
        "brief": (
            "你是执法文书专家。根据巡查要点与法规核验结果，生成检查记录框架与巡查清单："
            "要素完整（当事人/事实/证据/法律依据/裁量说明），用语规范。"
            "任务需要流程/结构图示时，用 ```show_widget 围栏输出 SVG 流程图"
            "（viewBox='0 0 680 <h>'，深色配色 #3dd9b6/#4da3e0，节点文字≤12字），"
            "不要输出 mermaid 代码块。"
        ),
        "max_tokens": 900,
    },
...
SYNTH_BRIEF = (
    ...
    "若专家产出含 ```show_widget 围栏，原样保留（前端内联渲染成图），"
    "禁止用表格/文字替换掉该围栏。"
    "用中文，结构清晰。"
)
...
def is_complex_task(text: str) -> bool:
    ...
    return bool(len(t) >= 30 and ("检查" in t or "执法" in t) and re.search(r"生成|出具|并.{0,6}(记录|清单|文书|报告)", t))


class RoleSwarm:
    """三角色协作总管"""

    def __init__(self, client=None, audit_chain=None, role_model: str = "", synth_model: str = ""):
        if client is None:
    ...
    def run(self, task: str, task_id: str = "", context: str = "", on_stage=None) -> dict:
        ...
        threads = [threading.Thread(target=_work, args=(r,), daemon=True) for r in ("patrol", "law")]
        for t in threads:
            t.start()
        for t in threads:
            t.join(timeout=180)
        ...
        try:
            ...
            lookup = _Path(__file__).resolve().parent.parent / "ecoskills" / "eco-codex" / "scripts" / "lookup.py"
            cited = sorted(
                {
                    int(x)
                    for x in re.findall(r"第\s*(\d{1,4})\s*条", contributions.get("law", ""))
                    if x.isdigit() and 1 <= int(x) <= 1242
                }
            )[:10]
            checks = []
            for n in cited:
                r = _sp.run([_sys.executable, str(lookup), "article", str(n)], capture_output=True, text=True, timeout=15)
                ok = r.stdout.strip().startswith("{")
                head = _json.loads(r.stdout).get("text", "")[:60] if ok else ""
                checks.append(f"- 第{n}条: {'✓ 存在' if ok else '✗ 查无'}" + (f"（{head}…）" if ok else ""))
            if checks:
                contributions["law"] = (
                    (contributions.get("law") or "") + "\n\n【法条核验】（自动工具校验）\n" + "\n".join(checks)
                )
        ...
        doc_ctx = "\n\n".join(
            f"[{ROLES[r]['name']}]\n{contributions.get(r, '')}" for r in ("patrol", "law") if contributions.get(r)
        )
        try:
        ...
        synth_input = "\n\n".join(f"【{ROLES[r]['name']}产出】\n{contributions.get(r, '（无产出）')}" for r in ROLE_ORDER)
        if hasattr(self.client, "chat"):
            resp = self.client.chat(
                [{"role": "system", "content": SYNTH_BRIEF}, {"role": "user", "content": f"任务：{task}\n\n{synth_input}"}],
