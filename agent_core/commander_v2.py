...
class TaskDecomposerV2:
    ...
    def __init__(self):
        ...
        self._patterns = {
            "开发": [
                ("分析需求", AgentRole.ANALYST, "产出需求清单，功能点与约束无遗漏"),
            ...
            "研究": [
                ("收集资料", AgentRole.RESEARCHER, "资料来源可溯源，覆盖主题主要方面"),
            ...
            "写作": [
                ("确定大纲", AgentRole.PLANNER, "大纲层级完整，覆盖主题要点"),
            ...
            "通用": [
                ("理解需求", AgentRole.ANALYST, "需求复述准确，关键约束无遗漏"),
    ...
    def decompose(self, goal: str, context: dict = None, skip: int = 0, note: str = "") -> list[Task]:
        """分解目标（B-01：≥5子任务，10秒内）
        ...
        for i, (desc, role, expectation) in enumerate(pattern):
            ...
            t = Task(
                description=f"{prefix}{desc}：{goal[:40]}" + (f"（{note}）" if note else ""),
                agent_role=role,
                priority=10 - i,
...
class Negotiator:
    ...
    def request(self, agent_id: str, resource: str, task_id: str) -> dict:
        ...
        if existing:
            return existing[0]
        entry = {"agent": agent_id, "task": task_id, "requested_at": time.time(), "position": len(self._pending[key])}
    ...
    def get_stats(self) -> dict:
        return {"active_contention": sum(len(v) for v in self._pending.values()), "resources": len(self._pending)}
...
class CommanderV2:
    ...
    def __init__(self, executor=None, verifier=None, replanner=None, max_mission_replans: int = 2):
    ...
    def _llm_verdict(task: Task) -> tuple[bool, str] | None:
        ...
        try:
            ...
            resp = c.complete(
                prompt, system="你是严格的验收员：只依据完成判据核验，证据不足一律判未达标，不臆测。", max_tokens=200
            )
            if not resp:
                return None
    ...
    def _default_replanner(self, goal: str, done: list[Task], failed: Task, remaining: list[Task]) -> list[Task]:
    ...
    def _run_waves(self, goal: str, tasks: list[Task]) -> list[Task]:
        ...
        while True:
            ...
            runnable = [
                ...
                and all(by_id.get(d) and by_id[d].status == TaskStatus.COMPLETED for d in t.depends_on)
            ]
            if not runnable:
                return tasks
            ...
            for t in runnable:
                t.input["upstream"] = {
                    by_id[d].description[:40]: by_id[d].output[:500] for d in t.depends_on if by_id[d].output
                }

            threads = [
                threading.Thread(target=self._execute_task, args=(t,), daemon=True) for t in runnable[:8]
            ]  # B-02：最多8并行
            for th in threads:
                th.start()
            for th in threads:
    ...
    def _replan_prefix(self, goal: str, tasks: list[Task], failed: Task) -> list[Task]:
        ...
        logger.info(
            f"  重规划 {self._mission_replans}/{self._max_mission_replans}："
    ...
    def get_stats(self) -> dict:
        return {"pool": self.pool.get_stats(), "missions": len(self._results), "negotiation": self.negotiator.get_stats()}
...
def test():
    ...
    print(
        f"[B-05] 协商: AgentA={'通过' if r_a['granted'] else '等待'}, AgentB={'通过' if r_b['granted'] else '等待'}", flush=True
    )
