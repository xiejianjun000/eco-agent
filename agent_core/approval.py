...
class ApprovalService:
    ...
    def __init__(self, policy: str | None = None, answerers: list[str] | None = None, path: Path | str | None = None):
    ...
    def _persist(self) -> None:
        if not self._writable:
            return
        try:
            lines = [json.dumps(r, ensure_ascii=False) for r in self._records.values()]
    ...
    def decide(self, request_id: str, allow: bool, answerer: str = "", reason: str = "") -> dict:
        """决定一条 pending 请求。answerer 必须命中链内成员（瀑布式）；
        链为空或 answerer 不在链内 → fail-closed 拒绝（即使 allow=True）。"""
        with self._lock:
            rec = self._records.get(request_id)
            if rec is None:
                return {"id": request_id, "status": _STATUS_NOT_FOUND, "allow": False, "reason": "请求不存在"}
            if rec["status"] != "pending":
                return {"id": request_id, "status": rec["status"], "allow": False, "reason": "请求已决定"}
    ...
    def list_pending(self) -> list[dict]:
        ...
        with self._lock:
            return [dict(r) for r in self._records.values() if r.get("status") == "pending"]
