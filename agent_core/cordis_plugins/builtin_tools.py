...
def _statute_handler(article: str = "", keyword: str = "", mode: str = "article") -> str:
...
def apply(ctx, config: dict | None = None) -> None:
    ...
    entries = [
        (
            ...
            {
                "type": "object",
                "properties": {"article": {"type": "string", "description": "条号（如1054）"}},
                "required": ["article"],
            },
            lambda **kw: _statute_handler(article=str(kw.get("article", "")), mode="article"),
        ),
        (
            "statute_search",
            "生态环境法典关键词检索——按关键词返回相关条文",
            {"type": "object", "properties": {"keyword": {"type": "string"}}, "required": ["keyword"]},
            lambda **kw: _statute_handler(keyword=str(kw.get("keyword", "")), mode="search"),
        ),
        (
            "tdocs_upload_html",
            "数据分析 HTML 报告一键上传为腾讯文档在线文档（aipage 打包+导入）",
            {"type": "object", "properties": {"path": {"type": "string"}, "title": {"type": "string"}}, "required": ["path"]},
            lambda **kw: _tdocs_handler(path=str(kw.get("path", "")), title=str(kw.get("title", ""))),
        ),
    ]
    registered: list[str] = []
    ...
    for name, desc, params, handler in entries:
        if name in _EXISTING:
            continue  # 已注册：幂等跳过
        try:
            register_external_tool(name, desc, params, handler, level="L1", category="内置能力")
            registered.append(name)
        except Exception as e:  # noqa: BLE001 — 幂等注册
            logger.debug("[builtin_tools] %s 注册跳过: %s", name, e)
    logger.info("[builtin_tools] 组合装配注册 %d 个内置工具: %s", len(registered), registered)
