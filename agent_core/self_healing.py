...
# 熔断器冷却时间基数（秒），可通过环境变量覆盖
_CIRCUIT_BASE_COOLDOWN = float(os.environ.get("ECO_CIRCUIT_COOLDOWN_BASE", "5"))
_CIRCUIT_MAX_COOLDOWN = float(os.environ.get("ECO_CIRCUIT_COOLDOWN_MAX", "300"))


class SelfHealer:
    """L5 韧性自愈循环——异常检测/分类/处理/恢复（线程安全版）"""

    def __init__(self):
        self._heal_count = 0
        self._fail_count = 0
        self._circuit_breakers: dict[str, dict] = {}
        self._lock = threading.Lock()  # 保护熔断器状态和计数器
    ...
    def protect(self, operation: Callable, context: str = "",
        ...
        with self._lock:
            ...
            if cb and cb["state"] == "open":
                remaining = cb["cooldown"] - (time.time() - cb["opened_at"])
                if remaining > 0:
                    logger.warning(f"[Heal] 熔断: {operation_name} (剩余{int(remaining)}s)")
                    return self._apply_fallback(operation_name, fallback, "熔断器开启")
                # 冷却期过，进入半开状态，仅允许一次探测
                cb["state"] = "half-open"
                cb["probe_pending"] = True
                logger.info(f"[Heal] {operation_name}: 半开探测启动")
            ...
            try:
                ...
                with self._lock:
                    ...
                    if operation_name in self._circuit_breakers:
                        ...
                        self._circuit_breakers[operation_name].pop("probe_pending", None)
        ...
        with self._lock:
            self._fail_count += 1
            prev_failures = self._circuit_breakers.get(operation_name, {}).get("failures", 0)
            # 真正的指数增长冷却：base * 2^failures，有上限
            cooldown = min(_CIRCUIT_MAX_COOLDOWN, _CIRCUIT_BASE_COOLDOWN * (2 ** prev_failures))
            ...
            logger.warning(f"[Heal] {operation_name}: 熔断器开启，冷却{cooldown:.0f}s "
                          f"(累计失败{prev_failures + 1}次)")
