...
_HANDSHAKE_KEYS = ("echostr", "challenge", "signature", "msg_signature", "timestamp", "nonce")
...
def dispatch_request(
    method: str, name: str, headers: dict | None = None, args: dict | None = None, body: bytes = b"", config: dict | None = None
) -> tuple[int, str, bytes]:
    ...
    if name not in CHANNELS:
        payload = {"error": f"未知渠道: {name}", "available": sorted(CHANNELS)}
    ...
    return 200, _JSON, json.dumps({"reply": result}, ensure_ascii=False).encode("utf-8")
...
class ChannelRequestHandler(BaseHTTPRequestHandler):
    ...
    def _sso_login(self) -> None:
        ...
        if not sso_mod.sso_enabled():
            self._send(404, _JSON, json.dumps({"error": "SSO 未启用（ECO_SSO）"}, ensure_ascii=False).encode("utf-8"))
            return
        try:
            ...
            self._send(502, _JSON, json.dumps({"error": f"SSO discovery 失败: {e}"}, ensure_ascii=False).encode("utf-8"))
            return
        self._redirect(url)
    ...
    def _sso_callback(self, args: dict) -> None:
        ...
        try:
            ...
            self._send(401, _JSON, json.dumps({"error": str(e)}, ensure_ascii=False).encode("utf-8"))
            return
        role = (
            sso_mod.map_role({cfg.role_claim: identity.get("role_claim", "")}, cfg.role_claim)
            if identity.get("role_claim")
            else sso_mod.rbac.Role.READONLY_VISITOR
        )
    ...
    def _cas_identity(provider, args: dict) -> dict:
        ...
        v = sso_mod.cas_validate(
            ticket, service=provider.config.redirect_uri, validate_url=provider.config.cas_validate_url, http_fn=provider._http
        )
    ...
    def _sso_guard_channels(self, method: str, args: dict) -> bool:
        ...
        self._send(
            401,
            _JSON,
            json.dumps({"error": "需要有效 SSO 会话（Authorization: Bearer <token>）"}, ensure_ascii=False).encode("utf-8"),
        )
        return True
    ...
    def _route(self, method: str) -> None:
        ...
        if len(parts) == 2 and parts[0] == "channels":
            ...
            status, ctype, payload = dispatch_request(
                method, name, headers=dict(self.headers.items()), args=args, body=body, config=self.channel_configs.get(name)
            )
            self._send(status, ctype, payload)
            return

        self._send(404, _JSON, json.dumps({"error": f"未知路径: {method} {path}"}, ensure_ascii=False).encode("utf-8"))
...
def make_server(host: str = "0.0.0.0", port: int = 7080, channel_configs: dict | None = None) -> ThreadingHTTPServer:
