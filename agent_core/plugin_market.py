...
class PluginMarket:
    ...
    def _load(self):
        if self._local_db.exists():
            try:
                self._plugins = json.loads(self._local_db.read_text("utf-8", errors="replace"))
            except Exception:
                pass
    ...
    def search(self, query: str, category: str = "") -> list[dict]:
        ...
        for p in self._plugins.values():
            ...
            if q in p["name"].lower() or q in p["description"].lower():
                results.append(
                    {k: p[k] for k in ["id", "name", "version", "description", "author", "category", "rating", "downloads"]}
                )
    ...
    def list_installed(self) -> list[dict]:
        ...
        for d in PLUGINS_DIR.iterdir():
            if d.is_dir() and (d / "plugin.json").exists():
                try:
                    results.append(json.loads((d / "plugin.json").read_text("utf-8")))
                except Exception:
                    pass
        return results
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            "by_category": by_cat,
            "avg_rating": round(sum(p["rating"] for p in self._plugins.values()) / max(len(self._plugins), 1), 1),
        }
    ...
    pid = pm.publish(
        {
            "name": "法规检索增强",
            "version": "1.0.0",
            "description": "增强版法规语义检索",
            "author": "eco-team",
            "category": "search",
            "files": ["search_enhancer.py"],
        }
    )
    pm.publish(
        {"name": "文书模板", "version": "0.5.0", "description": "执法文书模板库", "author": "eco-team", "category": "document"}
    )
    results = pm.search("法规")
    print(f"[Market] 搜索: {len(results)} 结果", flush=True)
    inst = pm.install(pid)
    print(f"[Market] 安装: {inst['success']}", flush=True)
    pm.rate(pid, 4.5)
    pm.rate(pid, 5.0)
    print(f"[Market] 评分: {pm._plugins[pid]['rating']}", flush=True)
    s = pm.get_stats()
    print(f"[Market] 总计: {s['total']}, 已安装: {s['installed']}", flush=True)
    assert s["total"] >= 2
    assert s["installed"] >= 1
    print("[PASS] Phase 5 插件市场测试通过", flush=True)
