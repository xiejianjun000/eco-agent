...
def build_bwrap_cmd(
    cmd: list[str], policy: SandboxPolicy, slirp: bool = False, unshare_pid: bool = True, mount_proc: bool = True
) -> list[str]:
    ...
    """
    args = ["bwrap", "--die-with-parent", "--new-session", "--unshare-ipc", "--unshare-uts"]
    if unshare_pid:
    ...
    if policy.network_allowlist:
        args += ["--setenv", "ECO_NET_ALLOWLIST", ",".join(policy.network_allowlist)]
...
def _run_degraded(cmd: list[str], policy: SandboxPolicy, reason: str) -> subprocess.CompletedProcess:
    ...
    try:
        ...
        def _limits():
            resource.setrlimit(resource.RLIMIT_CPU, (policy.max_seconds, policy.max_seconds + 5))
...
def _is_bwrap_launch_failure(result: subprocess.CompletedProcess, elapsed: float) -> bool:
    ...
    if stderr.startswith("bwrap:"):
        return True
    return result.returncode == 1 and not (result.stdout or "").strip() and not stderr and elapsed < _BWRAP_LAUNCH_FAIL_SECONDS


def run_in_sandbox(cmd: list[str], policy: SandboxPolicy | None = None) -> subprocess.CompletedProcess:
    ...
    if policy.network_allowlist and not slirp:
        log.warning(
            "network_allowlist=%s requested but slirp4netns missing; falling back to --unshare-net", policy.network_allowlist
        )

    last_err = ""
    for tier, (unshare_pid, mount_proc) in enumerate(_BWRAP_TIERS):
        wrapped = build_bwrap_cmd(cmd, policy, slirp=slirp, unshare_pid=unshare_pid, mount_proc=mount_proc)
        ...
        if not _is_bwrap_launch_failure(result, elapsed):
            ...
            if tier > 0:
                log.warning(
                    "os_sandbox bwrap 完整参数启动失败，已退化到 tier %d (unshare_pid=%s, mount_proc=%s) 执行成功",
                    tier,
                    unshare_pid,
                    mount_proc,
                )
        ...
        log.warning(
            "os_sandbox bwrap 启动级失败 (tier %d, unshare_pid=%s, mount_proc=%s): %s", tier, unshare_pid, mount_proc, last_err
        )

    r = _run_degraded(cmd, policy, reason=f"bwrap launch failed at all tiers ({last_err})")
    r.sandbox_mode = "degraded"
    return r
