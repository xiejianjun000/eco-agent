...
class SlackAdapter(ChannelAdapter):
    @property
    def platform(self) -> str: return "slack"
    ...
    def send_message(self, channel_id: str, message: str, reply_to: str = "") -> bool:
        ...
        try:
            r = requests.post("https://slack.com/api/chat.postMessage",
                ...
                json={"channel": channel_id, "text": message, "thread_ts": reply_to if reply_to else None}, timeout=10)
    ...
    def send_card(self, channel_id: str, title: str, content: str, actions: list[dict] = None) -> bool:
        ...
        if actions:
            blocks.append({"type": "actions", "elements": [{"type": "button", "text": {"type": "plain_text", "text": a.get("name","")}, "value": a.get("value","")} for a in actions[:5]]})
        r = requests.post("https://slack.com/api/chat.postMessage",
            headers={"Authorization": f"Bearer {self._token}", "Content-Type": "application/json"},
    ...
    def parse_webhook(self, raw_data: dict) -> UnifiedMessage | None:
        try:
            ...
            if not text:
                return None
            return UnifiedMessage(platform="slack", channel_id=event.get("channel",""),
                    user_id=event.get("user",""), content=text, msg_type=MessageType.TEXT, raw=raw_data)
        except Exception:
            return None


if __name__ == "__main__":
    bot = SlackAdapter(); print(f"[Slack] 已配置: {bot.is_configured()}")
