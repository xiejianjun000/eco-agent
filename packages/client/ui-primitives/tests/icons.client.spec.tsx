...
describe('EcoLogo', () => {
  it('renders the eco logo paths in the brand greens at the native ratio', () => {
    const { container } = render(<primitives.EcoLogo />)
    const svg = container.querySelector('svg')!
    expect(svg.getAttribute('viewBox')).toBe('0 0 788 348')
    expect(container.innerHTML).toContain('#0F766E')
    expect(container.innerHTML).toContain('#96D562')
  })
})
