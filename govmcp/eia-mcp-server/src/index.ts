...
const TOOL_DEFS = [
  { name: "kb_search", description: "搜索环评/排污许可法规知识库", inputSchema: { type: "object", properties: { query: { type: "string", description: "查询内容" }, topK: { type: "number", default: 5, description: "返回结果数量" }, filter: { type: "object", description: "元数据过滤条件" } }, required: ["query"] } },
  { name: "kb_verify", description: "验证具体条款的现行有效性", inputSchema: { type: "object", properties: { standardCode: { type: "string", description: "标准编号" }, clause: { type: "string", description: "条款内容" } }, required: ["standardCode"] } },
  { name: "kb_calculate", description: "执行标准计算（如排放量核算）", inputSchema: { type: "object", properties: { formula: { type: "string", description: "公式名称" }, params: { type: "object", description: "计算参数" } }, required: ["formula", "params"] } },
  { name: "kb_industry_info", description: "查询行业环评/排污许可要求", inputSchema: { type: "object", properties: { industryCode: { type: "string", description: "行业代码(GB/T4754)" }, queryType: { type: "string", enum: ["eia", "permit", "standards", "all"], default: "all" } }, required: ["industryCode"] } }
]
...
const server = new Server(
  ...
  {
    capabilities: {
      tools: (TOOL_DEFS.reduce((acc: any, tool) => { acc[tool.name] = { description: tool.description, inputSchema: tool.inputSchema }; return acc }, {}) as any)
    }
  }
)
...
// 工具清单（新版 SDK 需显式注册 ListToolsRequestSchema）
