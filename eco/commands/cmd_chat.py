...
def _filter_relevant_context(query: str, raw_context: str, max_chars: int = 400) -> str:
    """上下文相关性过滤：只保留与当前问题语义相关的记忆摘要
    
    避免之前的无关对话（如MCP安装）污染当前问题（如登录）的回答。
    """
    ...
    # 提取查询关键词（中文2字+ / 英文3字母+）
    import re
    q_keywords = set(re.findall(r"[\u4e00-\u9fff]{2,}|[A-Za-z]{3,}", query.lower()))
    if not q_keywords:
        return raw_context[:max_chars]
    
    # 按段落分割上下文，计算每段与查询的相关性
    paragraphs = raw_context.split("\n\n")
    scored = []
    for p in paragraphs:
        p_keywords = set(re.findall(r"[\u4e00-\u9fff]{2,}|[A-Za-z]{3,}", p.lower()))
        overlap = len(q_keywords & p_keywords)
        # 登录相关关键词特殊处理
        if "登录" in query or "login" in query.lower():
            if any(k in p for k in ["mcp", "clone", "git", "安装"]):
                overlap = 0  # 强制排除MCP安装相关内容
    ...
    for score, p in scored:
        if score > 0 or len(result) < 2:  # 至少保留2段（防止过度过滤）
            result.append(p)
            total += len(p)
            if total >= max_chars:
                break
...
def _repl(history=None):
    ...
    while True:
        ...
        ws = mgr.current()
        # 上下文相关性过滤：只保留与当前问题相关的记忆摘要
        raw_context = ws.summary() if ws else ""
        context = _filter_relevant_context(q, raw_context)
        from eco.trace import get_tracer
        tracer = get_tracer()
