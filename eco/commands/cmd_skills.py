...
def run(args):
    match args.action:
        ...
        case "install": return _install(args.name, force=getattr(args, "force", False))
        ...
        case "search": return _search(args.name)
        case "scan": return _scan(args.name)
        case "sign": return _sign(args.name)
...
def _install(path, force=False):
    ...
    print(f"[install] {result['name']} tier={result['trust_tier']} 验签={'通过' if result.get('verified') else '跳过'} 风险={scan.get('risk_level')}")
