...
def _build_parser():
    ...
    parser.add_argument("--version", "-V", action="version", version=f"eco {__version__}")
    ...
    p.add_argument(
        "--continue", "-c", dest="continue_session", action="store_true", help="恢复最近活跃工作区的会话历史继续对话"
    )
    p.add_argument("--resume", "-r", dest="resume", default=None, metavar="SLUG", help="按工作区名/slug 恢复指定会话历史")

    p = sub.add_parser("gateway", help="Manage gateway")
    p.add_argument("action", choices=["start", "stop", "restart", "status", "channels"])
    p.add_argument("channel_args", nargs="*", default=[], help="channels 子命令参数，如: eco gateway channels list")
    ...
    p.add_argument("action", choices=["list", "install", "remove", "info", "versions", "rollback", "search", "scan", "sign"])
    p.add_argument("name", nargs="?", default=None)
    p.add_argument("--force", action="store_true", help="强制安装 community 级技能或忽略高风险扫描结果")
    ...
    p.add_argument(
        "--otel", nargs="?", const="", default=None, metavar="OUT.json", help="导出 OTLP JSON（缺省写到 ./<session>.otlp.json）"
    )
    p.add_argument("--export", choices=["otlp"], default=None, help="直接 POST 到 OTel collector（失败降级写本地文件）")
    p.add_argument("--endpoint", default=None, metavar="URL", help="OTel collector endpoint（默认 http://localhost:4318）")
    ...
    p.add_argument("grant_id", nargs="?", default=None, help="revoke 的授权 id；或 sso 子操作（如: eco auth sso status）")
