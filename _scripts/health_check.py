...
def check() -> dict:
    ...
    try:
        ...
        rows.append({"group": "凭证配置", "name": cred["summary"], "ok": not cred["missing"],
    ...
    try:
        ...
        rows.append({"group": "工具注册", "name": f"govmcp 工具 {registry.count()} 个",
                     "ok": registry.count() >= 100, "detail": "三平台: " + platforms})
    except Exception as e:  # noqa: BLE001
    ...
    try:
        ...
        rows.append({"group": "聊天接线", "name": f"聊天工具 {len(chat_names)} 个（接线清单 {len(WIRED_REQUIRED)}）",
                     "ok": not missing, "detail": f"缺口: {missing}" if missing else "接线清单与聊天表一致"})
        if missing:
    ...
    try:
        ...
        ov = load_overrides()
        rows.append({"group": "权限闸门", "name": f"PERMISSION.md 覆盖 {len(ov)} 项",
                     "ok": len(ov) >= 20, "detail": "已加载" if ov else "未加载"})
    except Exception as e:  # noqa: BLE001
        rows.append({"group": "权限闸门", "name": "检查失败", "ok": False, "detail": str(e)})

    # ── 5. Web 构建产物 ──
    web_ok = (ROOT / "web" / "dist" / "index.html").exists()
    rows.append({"group": "Web UI", "name": "前端构建产物", "ok": web_ok,
    ...
    try:
        ...
        rows.append({"group": "提示词组装", "name": f"基础片段 {n} 个 + 注入 {len(eng.list_injections())} 条",
                     "ok": n >= 4, "detail": f"当前阶段: {eng.phase}"})
    except Exception as e:  # noqa: BLE001
    ...
    try:
        ...
        with urllib.request.urlopen("http://127.0.0.1:8321/healthz", timeout=3) as r:
            j = json.loads(r.read())
        rows.append({"group": "服务器", "name": "eco-server 8321", "ok": True,
                     "detail": f"status={j.get('status')} version={j.get('version')}"})
    except Exception:
        rows.append({"group": "服务器", "name": "eco-server 8321", "ok": False,
                     "detail": "未运行（python3 -m eco.cli server --port 8321）"})
        todos.append("启动服务器: python3 -m eco.cli server --port 8321")

    # ── 8. MCP 配置清单（B 维度接线可见性）──
    try:
        line = [l for l in (ROOT / ".env").read_text(encoding="utf-8").splitlines()
                if l.strip().startswith("ECO_MCP_SERVERS=")]
        servers = json.loads(line[0].split("=", 1)[1].strip()) if line else []
        rows.append({"group": "MCP 接线", "name": f"已配置 {len(servers)} 个 MCP",
                     "ok": len(servers) >= 3,
    ...
    try:
        ...
        rows.append({"group": "审计链", "name": f"SM3 审计链 {n} 条",
                     "ok": ok, "detail": "完整可验证" if ok else "链损坏"})
        if not ok:
    ...
    if ws and ecod and ws != ecod:
        rows.append({"group": "目录口径", "name": "工作区目录 ≠ 状态目录", "ok": True,
                     "detail": f"工作区(产物): {ws} | 状态(审计/会话): {ecod}——"
                               "两者用途不同属正常，但运行时上下文需同时标注避免误判"})
    else:
        rows.append({"group": "目录口径", "name": "目录口径", "ok": True,
                     "detail": f"统一目录: {ws or ecod or '默认 ~/.eco'}"})

    # ── 8. 平台连通性（仅 --live）──
    return {"rows": rows, "todos": todos,
            "summary": f"{sum(1 for r in rows if r['ok'])}/{len(rows)} 项通过"}
    ...
    for name, url in probes:
        try:
            ...
            with urllib.request.urlopen(req, timeout=8) as r:
                rows.append({"group": "连通性", "name": name, "ok": True,
                             "detail": f"HTTP {r.status}"})
        except urllib.error.HTTPError as e:
            if name == "腾讯文档 MCP 端点" and e.code == 401:
                rows.append({"group": "连通性", "name": name, "ok": False,
