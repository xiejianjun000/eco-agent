...
class ACEPipeline:
    ...
    def run(self, content: str, metadata: dict = None) -> dict:
        ...
        result = {
            ...
            "reflector": reflector,
            "curator": curator,
            "final_score": curator["score"],
            "passed": curator["passed"],
            "recommendation": curator["recommendation"],
    ...
    def _generate(self, content: str, metadata: dict) -> dict:
        ...
        return {
            ...
            "law_refs": self._extract_laws(content),
            "has_pointer": "## 原文指针" in content,
        }
    ...
    def _reflect(self, gen: dict) -> dict:
        ...
        if not checks["has_law_refs"]:
            issues.append("未发现法规引用")

        checks["has_pointer"] = gen["has_pointer"]
        if not checks["has_pointer"]:
            issues.append("缺少 ## 原文指针 段落")
    ...
    def _curate(self, gen: dict, ref: dict) -> dict:
        ...
        if score >= 90:
            ...
            rec, passed = "建议人工复核", True
        else:
            rec, passed = "退回修改", False
...
class SourcePointer:
    ...
    def check_file(self, content: str, file_path: str = "") -> dict:
        ...
        result["pointers"] = [line.strip().lstrip("- ") for line in result["pointer_section"].split("\n") if line.strip()]
...
class SkillUpgrader:
    ...
    def _load(self):
        if self._usage_log.exists():
            try:
                self._usage = json.loads(self._usage_log.read_text("utf-8", errors="replace"))
            except Exception:
                self._usage = {}
        else:
            self._usage = {"prompts": [], "upgraded": []}
...
"""
    ...
    def get_stats(self) -> dict:
        return {
            "total_prompts": len(self._usage["prompts"]),
