...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler(os.path.join(tempfile.gettempdir(), "docx_mcp_server.log")), logging.StreamHandler()],
)
...
class DocxProcessor:
    ...
    def _load_current_document(self):
        ...
        try:
            with open(CURRENT_DOC_FILE, encoding="utf-8") as f:
...
@mcp.tool()
def add_table(ctx: Context, rows: int, cols: int, data: list | None = None) -> str:
    """
    ...
    try:
            ...
            for r_idx, row in enumerate(table.rows):
                for c_idx, cell in enumerate(row.cells):
                    if keyword in cell.text:
                        results.append(
                            {"type": "table cell", "table_index": t_idx, "row": r_idx, "column": c_idx, "text": cell.text}
                        )
        ...
        for idx, result in enumerate(results):
            ...
            if result["type"] == "paragraph":
                ...
                response += (
                    f"in table {result['table_index']} at cell ({result['row']},{result['column']}): {result['text'][:100]}"
                )
                if len(result["text"]) > 100:
                    response += "..."
                response += "\n"
...
def search_and_replace(ctx: Context, keyword: str, replace_with: str, preview_only: bool = False) -> str:
    ...
    try:
        ...
        if preview_only:
            response += "This is a preview of replacements. No actual changes were made. To execute replacements, set preview_only to False."  # noqa: E501
        else:
...
@mcp.tool()
def merge_table_cells(ctx: Context, table_index: int, start_row: int, start_col: int, end_row: int, end_col: int) -> str:
    """
    Merge table cells
...
@mcp.tool()
def add_table_row(ctx: Context, table_index: int, data: list | None = None) -> str:
    """
    Add a row to table
...
def set_page_margins(
    ctx: Context, top: float | None = None, bottom: float | None = None, left: float | None = None, right: float | None = None
) -> str:
    """
    Set page margins
...
def replace_section(ctx: Context, section_title: str, new_content: list, preserve_title: bool = True) -> str:
    ...
    try:
        ...
        original_styles = []
        for i in range(
            start_delete := (title_index + (1 if preserve_title else 0)), min(end_index, start_delete + len(new_content))
        ):
            if i < len(doc.paragraphs):
                para = doc.paragraphs[i]
                style_info = {"style": para.style, "alignment": para.alignment, "runs": []}
...
def edit_section_by_keyword(ctx: Context, keyword: str, new_content: list, section_range: int = 3) -> str:
    """
    Find paragraphs containing specified keyword and replace them and their
    surrounding content, keeping original position, format, and style
    ...
    try:
        ...
        return f"Replaced paragraphs containing keyword '{keyword}' and their surrounding content, keeping original format and style"  # noqa: E501
    except Exception as e:
        error_msg = f"Failed to replace content: {str(e)}"
        logger.error(error_msg)
        traceback.print_exc()  # Print detailed error information
