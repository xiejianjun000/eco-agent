...
def test_cli_entry_help_exits_zero():
    ...
    r = subprocess.run(
        [sys.executable, "-m", "eco.cli", "server", "--help"], capture_output=True, text=True, timeout=60, cwd=ROOT
    )
...
def test_readme_mentions_cli_entry():
    ...
    assert "eco.cli" in readme or "eco server" in readme, "README 未提及主入口 `eco.cli server`"
    ...
    for py in ROOT.glob("agent_core/**/*.py"):
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef) and any(
                b.id in ("Exception", "RuntimeError", "ValueError", "OSError") for b in node.bases if isinstance(b, ast.Name)
            ):
...
def test_missing_api_key_graceful(monkeypatch):
    ...
    if isinstance(result, tuple):
        _, err = result
        assert err and ("api key" in err.lower() or "key" in err.lower()), f"错误消息不友好: {err}"
