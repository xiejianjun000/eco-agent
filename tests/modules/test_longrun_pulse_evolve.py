...
class TestShortPulseRun:
    def test_short_run_produces_jsonl_and_report(self, tmp_path):
        ...
        obs = LongRunObserver(duration_s=3.5, smoke=True, reports_dir=tmp_path,
                              pulse_interval_s=1, enable_evolve=False)
        out = obs.run()
    ...
    def test_run_start_records_claim_table_and_llm_flag(self, tmp_path):
        ...
        obs = LongRunObserver(duration_s=1.2, smoke=True, reports_dir=tmp_path,
                              pulse_interval_s=1, enable_evolve=False)
        out = obs.run()
...
class TestReportGeneration:
    ...
    def test_report_with_synthetic_evolve_and_anomaly(self, tmp_path):
        ...
        obs._emit({"type": "run_start", "llm_disabled": True, "pulse_interval_s": 2,
                   "pulse_adaptive_bounds": [1, 4]})
        obs._emit({"type": "evolve_cycle", "trigger": "script_manual（测试）", "elapsed_ms": 12.3,
                   "version": 7,
                   "phases": {"experience_replay": {"total_replayed": 0, "success_count": 0,
                                                  "fail_count": 0, "success_rate": "0%"},
                              ...
                              "self_versioning": {"version": 7, "snapshot_path": "/x/v7",
                                                  "retained_versions": 3}},
                   "llm_disabled": True,
                   "artifacts": {"report_path": "/x/evolution_report_v7.md", "report_exists": True,
                                 "version_snapshot": "/x/v7", "version_snapshot_exists": True,
                                 "skill_files_generated": "无", "memory_consolidation_files": "无"}})
    ...
    def test_heartbeat_gap_stats_in_report(self, tmp_path):
        ...
        for i, ts in enumerate(["2026-07-31T10:00:00", "2026-07-31T10:00:10"], 1):
            obs._emit({"type": "heartbeat", "pulse_id": f"pulse_{i}", "count": i,
                       "timestamp": ts, "elapsed_s": 0.01, "interval_s": 2,
