...
SOURCE = "大气污染防治法第九十九条：违反本法规定，超过大气污染物排放标准排放大气污染物的，由县级以上人民政府生态环境主管部门责令改正或者限制生产、停产整治，并处十万元以上一百万元以下的罚款。"
...
class TestRAGScorer:
    ...
    def test_hallucinated_answer_high_risk(self):
        ...
        scorer = RAGScorer()
        answer = ("根据大气污染防治法第八百八十八条，超标排放一律吊销营业执照，"
    ...
    def test_result_to_dict_complete(self):
        ...
        d = r.to_dict()
        assert {"faithfulness_score", "relevance_score", "completeness_score",
                "hallucination_risk", "overall_score"} <= set(d)
...
class TestFaithfulnessCheckTool:
    ...
    def test_check_with_inline_source(self):
        ...
        mcp = _load_mcp()
        resp = mcp.handle_tool_call(1, "eco_faithfulness_check", {
    ...
    def test_hallucinated_answer_flagged(self):
        ...
        content = json.loads(resp["result"]["content"][0]["text"])
        assert content["risk_level"] in ("medium", "high"), \
            f"幻觉答案未被标记: {content}"
