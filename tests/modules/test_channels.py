...
from agent_core.channels import BLOCK_TEXT, CHANNELS, Channel, InboundMessage, get_channel, handle_inbound
...
CFG = {
    "wecom": {"token": "tok-wecom-test", "encoding_aes_key": WECOM_AES_KEY},
...
def _wecom_post(text, msg_id="42"):
    ...
    sig = hashlib.sha1("".join(sorted(["tok-wecom-test", ts, nonce, enc])).encode()).hexdigest()
    return {"method": "POST", "args": {"msg_signature": sig, "timestamp": ts, "nonce": nonce}, "body": body}
...
def _dingtalk_req(text):
    ...
    body = {"msgtype": "text", "text": {"content": text}, "senderId": "ding-user-1", "msgId": "m1", "senderNick": "张三"}
    return {
        "method": "POST",
        "headers": {"timestamp": ts, "sign": sign("SECdingtalktest", ts)},
...
def _qqbot_req(text):
    ...
    body = json.dumps(
        {
            "op": 0,
            "d": {"content": text, "id": "qmsg-1", "author": {"id": "qq-user-1"}, "channel_id": "ch-9", "guild_id": "g-1"},
        }
    ).encode()
    ts = "1700000000.000000"
    key = Ed25519PrivateKey.from_private_bytes(_seed_from_secret("qqsecret"))
    sig = key.sign(ts.encode() + body).hex()
    return {"method": "POST", "headers": {"X-Signature-Ed25519": sig, "X-Signature-Timestamp": ts}, "body": body}
...
def _wechat_oa_req(text):
    ...
    sig = hashlib.sha1("".join(sorted(["oa-token-test", ts, nonce])).encode()).hexdigest()
    return {"method": "POST", "args": {"signature": sig, "timestamp": ts, "nonce": nonce}, "body": body}


def _webhook_req(text, secret="wh-shared-secret"):
    body = json.dumps({"user_id": "wh-user-1", "text": text, "msg_id": "w1"}).encode()
...
BUILDERS = {
    "wecom": _wecom_post,
    "dingtalk": _dingtalk_req,
    "feishu": _feishu_req,
    "qqbot": _qqbot_req,
    "wechat_oa": _wechat_oa_req,
    "webhook": _webhook_req,
}

EXPECTED_USER = {
    "wecom": "zhangsan",
    "dingtalk": "ding-user-1",
    "feishu": "ou_user1",
    "qqbot": "qq-user-1",
    "wechat_oa": "openid_user1",
    "webhook": "wh-user-1",
}
...
def test_registry_all_six_channels():
    assert set(CHANNELS) == {"wecom", "dingtalk", "feishu", "qqbot", "wechat_oa", "webhook"}
...
def test_wecom_url_verify_get_echostr():
    ...
    sig = hashlib.sha1("".join(sorted(["tok-wecom-test", ts, nonce, enc])).encode()).hexdigest()
    req = {"method": "GET", "args": {"msg_signature": sig, "timestamp": ts, "nonce": nonce, "echostr": enc}}
...
def test_wecom_reply_mocked():
    ch = get_channel("wecom", {**CFG["wecom"], "access_token": "tk", "agent_id": "1000002"})
    with mock.patch("agent_core.channels.wecom.http_post_json", return_value={"errcode": 0}) as p:
...
def test_dingtalk_reply_session_webhook_mocked():
    ...
    with mock.patch("agent_core.channels.dingtalk.http_post_json", return_value={"errcode": 0}) as p:
...
def test_qqbot_op11_ignored():
    ...
    req = {
        "method": "POST",
        "headers": {"X-Signature-Ed25519": key.sign(ts.encode() + body).hex(), "X-Signature-Timestamp": ts},
        "body": body,
    }
...
def test_qqbot_reply_mocked():
    ...
    with mock.patch("agent_core.channels.qqbot.http_post_json", return_value={"id": "msg-2"}) as p:
...
def test_wechat_oa_url_verify_get():
    ...
    sig = hashlib.sha1("".join(sorted(["oa-token-test", ts, nonce])).encode()).hexdigest()
    req = {"method": "GET", "args": {"signature": sig, "timestamp": ts, "nonce": nonce, "echostr": "echo-oa"}}
...
def test_wechat_oa_reply_mocked():
    ...
    with mock.patch("agent_core.channels.wechat_oa.http_post_json", return_value={"errcode": 0}) as p:
