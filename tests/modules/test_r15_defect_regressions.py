...
def _signed_webhook_body(text, secret=SECRET):
    body = json.dumps({"user_id": "u1001", "text": text},
...
class TestD4ChannelRoutes:
    ...
    def test_post_webhook_full_flow(self, gw, monkeypatch):
        ...
        status, ctype, payload = _http(f"{gw}/channels/webhook", "POST",
                                       body, headers)
        assert status == 200
    ...
    def test_post_injection_blocked(self, gw, monkeypatch):
        ...
        status, _, payload = _http(f"{gw}/channels/webhook", "POST",
                                   body, headers)
        assert status == 200
    ...
    def test_post_bad_signature_200_wording(self, gw, monkeypatch):
        ...
        status, _, payload = _http(f"{gw}/channels/webhook", "POST", body,
    ...
    def test_get_echostr_handshake_plain_text(self, gw, monkeypatch):
        ...
        monkeypatch.setattr(registry, "handle_inbound",
                            lambda name, req, config=None: "decrypted-echostr")
        status, ctype, payload = _http(
            f"{gw}/channels/wecom?msg_signature=x&timestamp=1&nonce=2&echostr=y")
        assert status == 200
    ...
    def test_get_feishu_challenge_json(self, gw, monkeypatch):
        ...
        monkeypatch.setattr(registry, "handle_inbound",
                            lambda name, req, config=None:
                            json.dumps({"challenge": "c-123"}))
        status, ctype, payload = _http(f"{gw}/channels/feishu", "POST",
    ...
    def test_dispatch_builds_request_dict(self, monkeypatch):
        ...
        seen = {}
        monkeypatch.setattr(registry, "handle_inbound",
                            lambda n, r, c=None: seen.update(req=r) or "ok")
        dispatch_request("POST", "webhook", headers={"x-signature": "sha256=aa"},
                         args={"a": "1"}, body=b"{}")
        req = seen["req"]
...
BUILD_SH = (REPO / "deploy" / "offline" / "build_offline.sh").read_text(
    encoding="utf-8")
...
class TestD1NonRootSystemdDegrade:
    ...
    def test_generated_install_sh_valid_bash(self):
        ...
        r = subprocess.run(["bash", "-n",
                            str(REPO / "deploy" / "offline" / "build_offline.sh")])
...
class TestD2EcoEntryInVenv:
    ...
    def test_pyproject_has_eco_script_entry(self):
        assert re.search(r"\[project\.scripts\][^\[]*eco\s*=\s*\"eco\.cli:main\"",
                         PYPROJECT)
...
class TestD5OtlpProxy:
    def test_default_bypasses_http_proxy(self, otlp_sink, monkeypatch,
                                         tmp_path):
    ...
    def test_default_opener_disables_proxy_handler(self, monkeypatch,
                                                   tmp_path):
        from agent_core.observability import OTLPExporter, SpanTree
        monkeypatch.delenv("ECO_OTLP_PROXY", raising=False)
        captured = {}
        ...
        tree.close_all()
        OTLPExporter(endpoint="http://127.0.0.1:4318",
                     fallback_dir=tmp_path).export(tree)
