...
class TestToolSchemaQuality:
    def test_no_zero_param_tools(self):
        zero = [t["function"]["name"] for t in tr.ALL_TOOL_DEFS if not t["function"].get("parameters", {}).get("properties")]
        assert zero == [], f"仍有零参数工具: {zero}"

    def test_no_template_descriptions(self):
        descs = [t["function"]["name"] for t in tr.get_tools() for _ in [t["function"].get("description", "")]]
