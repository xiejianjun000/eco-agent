...
DINGTALK_CFG = {"app_key": "ding_key", "app_secret": "ding_secret",
                "robot_code": "robot_1"}
...
def test_feishu_send_card_structure():
    ...
    with mock.patch("agent_core.channels.feishu.http_post_json",
        ...
        ok = ch.send_card("ou_user1", "执法风险操作审批", "详情内容",
                          approve_callback="cb-approve-1",
                          reject_callback="cb-reject-1")
    assert ok is True
...
def test_feishu_tenant_token_cache_reused():
    ch = get_channel("feishu", FEISHU_CFG)
    with mock.patch("agent_core.channels.feishu.http_post_json",
                    return_value={"code": 0, "tenant_access_token": "tat-c",
                                  "expire": 7200}) as p:
...
def _wecom_mocks(token_resp=None, send_resp=None):
    token_resp = token_resp or {"errcode": 0, "access_token": "wct-1",
                                "expires_in": 7200}
    send_resp = send_resp or {"errcode": 0}
    return (mock.patch("agent_core.channels.wecom.WeComChannel._get_token",
                       return_value=token_resp),
            mock.patch("agent_core.channels.wecom.http_post_json",
                       return_value=send_resp))
...
def test_wecom_send_text_card():
    ...
    with tok, post as p:
        assert ch.send_text_card("zhangsan", "标题", "描述",
                                 url="https://example.test/detail") is True
...
def test_wecom_create_approval_payload():
    ch = get_channel("wecom", WECOM_CFG)
    tok, post = _wecom_mocks(
        send_resp={"errcode": 0, "sp_no": "202401010001"})
    with tok, post as p:
        sp_no = ch.create_approval("creator1", ["approver1", "approver2"],
                                   "Tpl-001", {"contents": []})
    ...
    assert payload["approver"] == [{"attr": 0, "userid": ["approver1",
                                                          "approver2"]}]
...
def test_dingtalk_single_vs_group_message():
    ...
    with mock.patch("agent_core.channels.dingtalk.http_post_json",
                    side_effect=_dingtalk_post_factory(calls)):
...
def test_dingtalk_query_approval_uses_oapi_token():
    ch = get_channel("dingtalk", DINGTALK_CFG)
    calls = []
    with mock.patch("agent_core.channels.dingtalk.http_get_json",
                    return_value={"errcode": 0, "access_token": "oat-1",
                                  "expires_in": 7200}) as g, \
            mock.patch("agent_core.channels.dingtalk.http_post_json",
                       side_effect=lambda url, payload, headers=None, timeout=10:
                       calls.append((url, payload)) or
                       {"errcode": 0,
                        "process_instance": {"id": "pi-1", "status": "RUNNING"}}):
...
def test_dingtalk_app_token_cache_reused():
    ch = get_channel("dingtalk", DINGTALK_CFG)
    with mock.patch("agent_core.channels.dingtalk.http_get_json",
                    return_value={"errcode": 0, "access_token": "oat-c",
                                  "expires_in": 7200}) as g:
