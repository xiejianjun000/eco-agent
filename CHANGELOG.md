...
### 背景
远程 EHS 知识库 MCP（`http://111.230.89.107:8000/sse`）已不可达（403 / TaskGroup 异常），
原 `--rag` 模式整体失效，只能跑无检索 baseline。而法典全文本就在本地
`ecoskills/eco-codex/kb/`（五编 1242 条），无需依赖远程服务。

### 本轮 baseline 实测（deepseek-chat，70 题）
引用准确率 0.4295 / 要点 F1 0.6021。法典五类几乎全线归零，两种失分性质完全不同：
- **知识缺口**：EB57/EB65 直接回答「我国尚未制定《生态环境法典》」（模型训练截止早于颁布）；
- **幻觉**：EB66 自信编造「第九编…第一千一百八十九条」——法典实为**五编**，正确条号 **1108**。

### Added
- **`LocalCodexRetriever`**（`benchmarks/ecobench/run_ecobench.py`）：直读本地法典全文，
  与 `RagRetriever` 保持同一契约 `retrieve_v2(item) -> {files, articles, context}`。
  kb 正文条款是裸行（`第一千一百零八条　…`）而非 markdown 标题，故用专用切分建 1242 条索引。
  编章骨架输出五编＋条号区间（第 1-147 / 148-673 / 674-937 / 938-1051 / 1052-1242），
  作为「第九编」幻觉的纠偏锚点。
- 新增 `--local-rag` 开关；mode 标记区分 `rag-local` / `rag-remote`。
- `tests/modules/test_local_codex_retriever.py`：9 项回归（条文精确取用、五编骨架、
  跨法过滤、骨架不越界、`retrieve_v2` 契约），全绿。

### Fixed（两个在实测中暴露并逐一定位的检索缺陷）
- **跨法条号串味**：`required_citations` 常混《行政处罚法》《立法法》等外部法条号，
  原实现照单在法典库里取，把「法典第三十七条（资源总量管理/资源节约）」当成
  「《行政处罚法》第三十七条」注入。实测使「法典新旧衔接」从 0.55 掉到 0.45。
  修复：`_codex_article_nums` 按《法名》过滤，只取归属法典的条号。
- **骨架噪声**：修上一条后骨架兜底对每道非法典题都触发，给《固废法》《大气法》题
  注入法典编章结构纯属干扰，「违法认定」被打到 0.20。
  修复：仅当类别/题干/引用确与法典相关时才注入骨架；其余 40 题回退为无检索（=baseline 行为）。

### 结果（70 题全量，如实报告，无封顶/保底）

| 类别 | baseline | local-RAG | Δ |
|---|---|---|---|
| 法典-引用规范 | 0.00 | 1.00 | +1.00 |
| 法典-新旧衔接 | 0.00 | 1.00 | +1.00 |
| 法典-继承映射 | 0.07 | 0.73 | +0.67 |
| 法典-框架结构 | 0.25 | 0.80 | +0.55 |
| 执法程序 | 0.40 | 0.60 | +0.20 |
| 处罚裁量 | 0.70 | 0.80 | +0.10 |
| 法条引用 | 0.70 | 0.80 | +0.10 |
| 法典新旧衔接 | 0.55 | 0.65 | +0.10 |
| 违法认定 | 0.50 | 0.45 | -0.05 |
| **总计** | **0.4295** | **0.7238** | **+0.2943** |
| 要点 F1 | 0.6021 | 0.7276 | +0.1255 |

报告：`report_baseline_20260908.json` / `report_localrag_v3_20260908.json`
（v1/v2 为修复过程中的中间轮次，一并留存以备复核）。

### 待办
- 「违法认定」0.45、「法典新旧衔接」0.65 仍偏低：这两类多为**外部单行法**条号，
  本地只有法典全文，需补齐单行法库或恢复远程 EHS 知识库才能继续提升。
- 远程 EHS 知识库不可达，`--rag` 模式当前无法验证。

## [2026-09-08] 监测技术规范清单穿透式全量学习（209 项标准 + 8 份法规全文）

### Added
- **新技能 `ecoskills/jiance-guifan/`（自审 100/100）**：生态环境监测技术规范速查。
  知识库由《生态环境监测技术规范清单（2026.7）》PDF 原件（88 页）逐页抽取：
  - `kb/监测技术规范清单_结构化.json|.jsonl`：**209 项**标准，含要素/监测方式/标准序列/
    强制性/同名多版本/溯源页码等 17 个字段；
  - `kb/监测配套法规全文.json|.md`：附录 **8 份法规共 205 条**全文（生态环境监测条例 49 条、
    资质认定管理办法 40、监督管理办法 29、评审准则 21、生态环境监测机构评审补充要求 2025 24、
    其条文释义 24、数据弄虚作假判定办法 18，另深化监测改革意见）；
  - `kb/学习题库.json|.jsonl`：**496 题**（标准定位 209／法规条文 203／标准识别 60／
    时效风险 15／罚则定位 5／版本辨析 4），每题带 answer_key 可机械判分；
  - `scripts/lookup.py`：std／search／medium／nav／law／lawsearch／stats 七类检索，
    默认 JSON、`--text` 人读；SKILL.md 明令禁止整读 kb（19 万字会爆上下文）。
- **evals 新增两个穿透式套件**：`evals/jiance-standards.md`（209 题）、
  `evals/jiance-laws.md`（208 题）；`_scripts/run_evals.py` 新增 `std=` 与 `jcfa=`
  两类机械校验（直查 jiance-guifan 知识库，标准号/条号不存在即判虚构引用）。
  全量机械校验 **452/452 通过**（原 35 题 + 新增 417 题）。
- `tests/modules/test_jiance_guifan.py`：15 项回归（知识库完整性、分节序号连续性、
  字形缺陷修复点、七类检索、反幻觉查无、题库与套件完整性），全绿。

### Fixed（原件字形缺陷——差点造成条号错位引用）
- PDF 存在字形替换缺陷：条号中的「十」被以 ArialMT／SimSun-ExtB 的 `H` 输出，
  且紧随其后的个位数字**整格丢失**（x 轴留一个字宽空档、无 span）。直接文本抽取会得到
  「第三H条」并丢失整条，导致《生态环境监测条例》第三十一/四十一条、
  《检验检测机构监督管理办法》第二十一条**凭空消失**。
  已按字形几何（空档字宽探测）+ 条序还原，并与 mee.gov.cn 权威全文交叉比对：
  **《生态环境监测条例》49/49 条逐条一致**。
- 抽取审计：209 条记录的名称/标准号/发布日期全部回溯原文命中，各分节序号 1..N
  连续无缺（AUDIT PASS）；另随机抽检 25 条回溯 PDF 原页三字段，25/25 命中。

### Notes
- 同批分享的《环境监测（第六版，奚旦立）》教材（179MB）受夸克匿名下载体积上限
  （code 23018 download file size limit）阻断，未取回；需登录态或分卷方可获取。
