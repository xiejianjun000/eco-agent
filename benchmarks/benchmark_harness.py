...
class BenchmarkRunner:
    ...
    def _external_placeholder(self, bid: str, info: dict) -> dict:
        ...
        return {"name": info["name"], "score": None, "target": info["target"],
                "passed": None, "unit": info["unit"], "status": "not_run",
    ...
    def _run_token_compression(self) -> dict:
        ...
        return {"name": "TokenCompression+RAG(内部实测)", "score": acc,
                "compression_ratio": r["ratio"], "target": 0.9,
                "passed": acc >= 0.9, "unit": "关键词保留率", "status": "measured"}
    ...
    def report(self) -> str:
        ...
        for bid, info in {**EXTERNAL_BENCHMARKS,
                          "token_compression": {"name": "TokenCompression+RAG(内部实测)",
                                                "target": 0.9, "unit": "关键词保留率"}}.items():
            latest = data.get(bid)
            if not latest:
